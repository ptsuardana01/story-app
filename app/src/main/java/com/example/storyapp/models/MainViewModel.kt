...
class MainViewModel(storyRepository : StoryRepository, private val pref: AuthPreferences) : ViewModel() {
    ...
    val msgUpload: LiveData<String> = _msgUpload
    ...
    fun addNewStory(desc: RequestBody, photo: MultipartBody.Part, token: String, lat: Float?, lon: Float?) {
        ...
        val client = ApiConfig.getApiService("Bearer $token").addNewStory(photo, desc, lat, lon)
        client.enqueue(object : Callback<AddNewStoryResponse> {
            override fun onResponse(
                ...
                _isLoading.value = false
                if (response.isSuccessful) {
                    _msgUpload.value = response.message().toString()
                }
            }

            override fun onFailure(call: Call<AddNewStoryResponse>, t: Throwable) {
                _isLoading.value = false
                _msgUpload.value = t.message.toString()
            }
